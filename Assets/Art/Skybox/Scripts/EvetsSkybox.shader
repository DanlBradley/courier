    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Fragment(Varyings v) : SV_TARGET
            {
                ...
                // Map time to gradient position based on sun position
                // The sun angle formula: (currentTimeOfDay - 6f) * 15f
                // So sun is at -90° at midnight, 0° at 6AM, 90° at noon, 180° at 6PM, 270° at midnight
                // We want gradient to match sun height: 0 = full night, 0.5 = noon, 1 = full night
                float sunHourAngle = (_TimeOfDay * 24.0 - 6.0) * 15.0; // Convert to degrees
                float sunHeight = sin(sunHourAngle * 3.14159265 / 180.0); // -1 to 1
                
                // Map sun height to gradient position
                // sunHeight -1 (midnight) -> 0
                // sunHeight 0 (sunrise/sunset) -> 0.5  
                // sunHeight 1 (noon) -> 1
                // Then wrap so both ends (0 and 1) represent night
                float gradientTime = saturate((sunHeight + 1.0) * 0.5);

                // sky colors - use mapped time for gradient sampling
                float3 sunZenithColor = SAMPLE_TEXTURE2D(_SunZenithGrad, sampler_SunZenithGrad, float2(gradientTime, 0.5)).rgb;
                float3 viewZenithColor = SAMPLE_TEXTURE2D(_ViewZenithGrad, sampler_ViewZenithGrad, float2(gradientTime, 0.5)).rgb;
                float vzMask = pow(saturate(1.0 - viewZenithDot), 4);
                float3 sunViewColor = SAMPLE_TEXTURE2D(_SunViewGrad, sampler_SunViewGrad, float2(gradientTime, 0.5)).rgb;
                ...
                float3 sunOverlayColor = _SunColorCustomize == 0 ? _MainLightColor.rgb : SAMPLE_TEXTURE2D(_SunColorGrad, sampler_SunColorGrad, float2(gradientTime, 0.5)).rgb;
                ...
                float3 cloudRawColor = SAMPLE_TEXTURE2D(_CloudGrad, sampler_CloudGrad, float2(gradientTime, 0.5)).rgb;
